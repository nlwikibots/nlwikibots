...
sys.path.append(os.environ['HOME'] + "/lib/pywikipedia")
...
class AfDBot:
    ...
    def run(self):
        ...
        if self.db:
            # Get replag
            sql =   """
                    SELECT time_to_sec(timediff(now()+0,rev_timestamp)) AS replag
    ...
    def treatUser(self, page):
        ...
        if not user in self.contributors.keys():
            ...
            comment = u'Nieuw onderwerp: /* %s */ Automatische melding van verwijderingsnominatie' % header
